...
module.exports = class extends Provider {
  ...
  async init() {
    ...
    /* Guilds cache */
    this.getAll('guilds').then((guilds) => {
      const guildsMap = new Map(guilds.map((guild) => {
        if (guild.roleReaction) {
          const roleReactionMap = new Map(Object.entries(guild.roleReaction));
          roleReactionMap.forEach((value, key) => {
            roleReactionMap.set(key, {
              id: value.id,
              rolesEmojiMap: new Map(Object.entries(value.rolesEmojiMap)),
            });
          });
          const newGuildObject = Object.assign(guild, { roleReaction: roleReactionMap });
          return [newGuildObject.id, newGuildObject];
        }
